<template>
    <Row jc="between" v-if="pages">
        <Col :md="6" :sm="12" class="text-muted small">
            Showing {{ pages.data.length }} items from {{ pages.from }} to {{ pages.to }} of {{ pages.total }}
            items
        </Col>
        <Col :md="6" :sm="12">
            <Pagination size="sm" :class="paginationClass" class="justify-content-md-end">
...
defineProps({
    pages: {type: Object as PropType<Datatable<any>>},
    paginationClass: {
        default: "mb-0"
    }
});
